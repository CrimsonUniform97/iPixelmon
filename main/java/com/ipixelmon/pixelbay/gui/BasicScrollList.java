...
package com.ipixelmon.pixelbay.gui;
...
public abstract class BasicScrollList
{
    ...
    public BasicScrollList(Minecraft client, int width, int height, int top, int bottom, int left, int entryHeight, int screenWidth, int screenHeight)
    {
        this.client = client;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        for (int slotIdx = 0; slotIdx < listLength; ++slotIdx)
        {
            ...
            if (slotTop <= this.bottom && slotTop + slotBuffer >= this.top)
            {
                ...
                {
                    this.drawSelectionBox(this.left, slotTop, entryRight, slotBuffer);
        ...
        if (extraHeight > 0)
        {
            ...
            drawScrollBar(scrollBarLeft, scrollBarRight, barTop, height);
        }
    ...
    public void drawSelectionBox(int slotLeft, int slotTop, int slotRight, int slotBuffer)
    {
        Tessellator tess = Tessellator.getInstance();
        WorldRenderer worldr = tess.getWorldRenderer();
        int min = this.left;
    ...
    public void drawScrollBar(int scrollBarLeft, int scrollBarRight, int thumbTop, int thumbHeight)
    {
        ...
        tess.draw();
        worldr.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        worldr.pos(scrollBarLeft, thumbTop + thumbHeight, 0.0D).tex(0.0D, 1.0D).color(0x80, 0x80, 0x80, 0xFF).endVertex();
        worldr.pos(scrollBarRight, thumbTop + thumbHeight, 0.0D).tex(1.0D, 1.0D).color(0x80, 0x80, 0x80, 0xFF).endVertex();
        worldr.pos(scrollBarRight, thumbTop, 0.0D).tex(1.0D, 0.0D).color(0x80, 0x80, 0x80, 0xFF).endVertex();
        worldr.pos(scrollBarLeft, thumbTop, 0.0D).tex(0.0D, 0.0D).color(0x80, 0x80, 0x80, 0xFF).endVertex();
        tess.draw();
        worldr.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        worldr.pos(scrollBarLeft, thumbTop + thumbHeight - 1, 0.0D).tex(0.0D, 1.0D).color(0xC0, 0xC0, 0xC0, 0xFF).endVertex();
        worldr.pos(scrollBarRight - 1, thumbTop + thumbHeight - 1, 0.0D).tex(1.0D, 1.0D).color(0xC0, 0xC0, 0xC0, 0xFF).endVertex();
        worldr.pos(scrollBarRight - 1, thumbTop, 0.0D).tex(1.0D, 0.0D).color(0xC0, 0xC0, 0xC0, 0xFF).endVertex();
        worldr.pos(scrollBarLeft, thumbTop, 0.0D).tex(0.0D, 0.0D).color(0xC0, 0xC0, 0xC0, 0xFF).endVertex();
        tess.draw();
    }
